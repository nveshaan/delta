"""MRI raw-data dataset factory; see ``configs/datasets/raw_mri.yaml``."""
...
class MRIRawDataset(RawDataset):
...
def build_dataset(config: Mapping[str, Any], split: str | None = None, transform: Transform = None) -> MRIRawDataset:
    """Build the flat configured source collection."""
    split = split or "sources"
    section = select_split(config, split)
    return MRIRawDataset(section["sources"], transform=transform, seed=int(config.get("seed", 42)),
...
def main() -> None:
    ...
    config_path = Path(__file__).resolve().parents[1] / "configs" / "datasets" / "raw_mri.yaml"
    ...
    print(f"mri: image batch shape={tuple(images.shape)}, labels shape={tuple(labels.shape)}")
