"""Fundus raw-data dataset factory; see ``configs/datasets/raw_fundus.yaml``."""
...
class FundusRawDataset(RawDataset):
    ...
    def __init__(self, sources=None, transform: Transform = None, seed: int = 42,
                 sample_strategy: str = "python_ordered", image_extensions=tuple(IMAGE_EXTENSIONS),
...
def build_dataset(config: Mapping[str, Any], split: str | None = None, transform: Transform = None) -> FundusRawDataset:
    """Build the flat configured source collection."""
    split = split or "sources"
    section = select_split(config, split)
    return FundusRawDataset(section["sources"], transform=transform, seed=int(config.get("seed", 42)),
                            sample_strategy=config.get("sample_strategy", "python_ordered"),
...
def main() -> None:
    ...
    config_path = Path(__file__).resolve().parents[1] / "configs" / "datasets" / "raw_fundus.yaml"
    ...
    print(f"fundus: image batch shape={tuple(images.shape)}, labels shape={tuple(labels.shape)}")
