"""OCT raw-data dataset factory; see ``configs/datasets/raw_oct.yaml``."""
...
class OCTRawDataset(RawDataset):
    ...
    def __init__(self, sources=None, transform: Transform = None, seed: int = 42,
                 sample_strategy: str = "numpy_sorted", image_extensions=tuple(IMAGE_EXTENSIONS),
...
def build_dataset(config: Mapping[str, Any], split: str | None = None, transform: Transform = None) -> OCTRawDataset:
    """Build the flat configured source collection."""
    split = split or "sources"
    section = select_split(config, split)
    return OCTRawDataset(section["sources"], transform=transform, seed=int(config.get("seed", 42)),
                         sample_strategy=config.get("sample_strategy", "numpy_sorted"),
...
def main() -> None:
    ...
    config_path = Path(__file__).resolve().parents[1] / "configs" / "datasets" / "raw_oct.yaml"
    ...
    print(f"oct: image batch shape={tuple(images.shape)}, labels shape={tuple(labels.shape)}")
